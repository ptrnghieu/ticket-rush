...
import { fmtDate, fmtVND, statusLabel, statusBadgeClass } from '../utils/format';
...
export default function Event() {
  ...
  if (error)   return <div className="container" style={{ padding: 'var(--sp-8)' }}><div className="alert alert-error">{error}</div></div>;
  ...
  return (
    <>
      ...
      <div className="event-hero">
        {event.poster_url
          ? <><img className="event-hero__img" src={event.poster_url} alt={event.name} /><div className="event-hero__overlay" /></>
      ...
      <div className="container">
        ...
        <div className="event-header">
          <div style={{ marginBottom: 'var(--sp-3)' }}>
            <span className={`badge ${statusBadgeClass(event.status)}`}>{statusLabel(event.status)}</span>
          </div>
          ...
          {event.description && (
            <p style={{ marginTop: 'var(--sp-4)', color: 'var(--text-2)', fontSize: 'var(--text-sm)', lineHeight: 1.7, maxWidth: 720 }}>
              {event.description}
            </p>
          )}
        </div>
          ...
          <div className="event-layout">
            ...
            <div>
              <div className="selection-panel">
                  ...
                  <>
                    {selection.seats.map(seat => (
                      <div key={seat.id} className="selected-seat-item">
                        <span>{seat.row_label ?? seat.row_number}{seat.seat_number} — {seat.sectionName}</span>
                        <span style={{ color: 'var(--accent)' }}>{fmtVND(seat.price)}</span>
                      </div>
                    ))}
                ...
                {lockError && <div className="alert alert-error" style={{ marginTop: 'var(--sp-3)', fontSize: 'var(--text-xs)' }}>{lockError}</div>}
