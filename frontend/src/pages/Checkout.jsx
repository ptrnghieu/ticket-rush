...
export default function Checkout() {
  ...
  if (!event || !seats.length) {
    return (
      <div className="container" style={{ padding: 'var(--sp-8)' }}>
        <div className="alert alert-error">Không có thông tin đặt vé. <a href="/" className="btn btn-ghost btn-sm" style={{ marginLeft: 8 }}>Về trang chủ</a></div>
      </div>
    );
  }
  ...
  async function handlePay() {
    ...
    try {
      ...
      setError(err.response?.data?.detail ?? 'Thanh toán thất bại. Vui lòng thử lại.');
      setStep('error');
    }
  }

  async function handleCancel() {
    if (!order) { navigate(`/events/${event.id}`); return; }
    setCancelling(true);
    try {
  ...
  if (step === 'success') {
    return (
      <div style={{ textAlign: 'center', padding: 'var(--sp-16) var(--sp-6)' }}>
        ...
        <h1 style={{ fontSize: 'var(--text-2xl)', fontWeight: 800, marginBottom: 'var(--sp-3)' }}>
          Đặt vé thành công!
        </h1>
  ...
  return (
    <div className="container" style={{ padding: 'var(--sp-8) var(--sp-6)', maxWidth: 640 }}>
      <h1 style={{ fontSize: 'var(--text-2xl)', fontWeight: 800, marginBottom: 'var(--sp-6)' }}>
        Xác nhận đặt vé
      </h1>

      {/* Event info */}
      <div className="card" style={{ marginBottom: 'var(--sp-4)' }}>
        <h2 style={{ fontWeight: 700, marginBottom: 'var(--sp-2)' }}>{event.name}</h2>
      ...
      <div className="checkout-summary" style={{ marginBottom: 'var(--sp-6)' }}>
        <h3 style={{ fontWeight: 700, marginBottom: 'var(--sp-3)' }}>Chi tiết đơn hàng</h3>
        {seats.map(seat => (
          <div key={seat.id} className="checkout-row">
            <span>Ghế {seat.row_label ?? seat.row_number}{seat.seat_number} — {seat.sectionName}</span>
      ...
      {step === 'error' && (
        <div className="alert alert-error" style={{ marginBottom: 'var(--sp-4)' }}>{error}</div>
      )}
      ...
      <p style={{ fontSize: 'var(--text-xs)', color: 'var(--text-3)', marginTop: 'var(--sp-3)', textAlign: 'center' }}>
        Ghế sẽ được giữ đến khi thanh toán. Hủy sẽ giải phóng ghế cho người khác.
      </p>
    </div>
  );
}
